...
redis_customer_json_schema = StructType([
...
spark = SparkSession.builder.appName('kafka_join').getOrCreate()
...
df_redis = spark.readStream.format('kafka')\
...
df_redis= df_redis.selectExpr('CAST(value as string) value')
...
df_redis.withColumn('value', from_json('value', redis_topic_schema))\
...
df_customer = spark.sql('SELECT zSetEntries[0].element as encodedCustomer FROM RedisSortedSet')
...
df_customer = df_customer.withColumn('customer', unbase64(df_customer.encodedCustomer).cast('string'))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
df_customer.withColumn('customer', from_json('customer', redis_customer_json_schema)).select('customer.*').createOrReplaceTempView('CustomerRecords')
...
sql = 'SELECT * FROM CustomerRecords WHERE email IS NOT NULL AND birthDay IS NOT NULL'
emailAndBirthDayStreamingDF = spark.sql(sql)
...
df_stedi.select(col('value.customer'),col('value.score'),col('value.riskDate')).createOrReplaceTempView('CustomerRisk')
...
df_join = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF,expr('customer = email'))
...
df_join_json = df_join.selectExpr('TO_JSON(struct(*)) as value')

kafka_output = df_join_json.writeStream.outputMode('append').format('kafka')\
.option('kafka.bootstrap.servers','localhost:9092')\
.option('startingOffsets','earliest')\
.option("FailOnDataLoss" , "false") \
.option("checkpointLocation", "checkpoint") \
.option('topic','stedi-risk-graph')\
.start()

console_output = df_join.writeStream.outputMode('append').format('console').start()
